...
class SPN:
    ...
    def dnuor(self, input_val, cur_round):
        ...
        if cur_round != self.n_rounds - 1:
            input_val = self.permute(input_val)
    ...
    def _substitute(self, message, sub_dict):
        ...
        for idx in range(0, int(self.BITS_SIZE / 4)):
            ...
            verbose_print(
                f"idx: {idx}, mask:{mask:x} cur_val: {cur_val:x}, lookup:{cur_val >> shift:x}, replace:{sub_dict[cur_val >> shift]}, ret_val: {ret_val:x}")
            mask <<= 4
    ...
    def permute(self, message):
        ...
        for idx in range(1, self.BITS_SIZE + 1):
            ...
            if curBit:
                ...
                ret_val |= location
                verbose_print(
                    f"permute:: idx: {idx}, to:{(self.perm_lookup[idx])}, bit_val: {curBit}, shift: {shift}, mask: {mask:x}, location: {location:b} ret_val:{ret_val:b}")
